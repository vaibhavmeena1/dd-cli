...
// A successful catalog request is repeated at most every six hours; a failed
// one is retried after fifteen minutes. Explicit setup bypasses both.
export const PI_MCP_SYNC_SUCCESS_INTERVAL_MS = 6 * 60 * 60_000;
export const PI_MCP_SYNC_FAILURE_INTERVAL_MS = 15 * 60_000;

const syncStampSchema = z.strictObject({
...
export type PiMcpSyncStamp = z.infer<typeof syncStampSchema>;
...
export async function synchronizePiMcpConfig(
  ...
  if (options.stampPath !== undefined && options.force !== true) {
    const stamp = await readSyncStamp(options.stampPath);
    if (isMcpSyncThrottled(stamp, sourceUrl, now)) {
      return result("throttled", configPath, sourceUrl, []);
    }
  }
  ...
  if (options.stampPath !== undefined) {
    await writeSyncStamp(options.stampPath, {
      schemaVersion: 1,
      lastAttemptAt: now.toISOString(),
      lastStatus: remote.servers === undefined ? "failed" : "ok",
      sourceUrl,
    });
  }
...
async function readSyncStamp(stampPath: string): Promise<PiMcpSyncStamp | undefined> {
  try {
    const status = await lstat(stampPath);
    if (!status.isFile() || status.isSymbolicLink()) {
      return undefined;
    }
    return syncStampSchema.parse(JSON.parse(await Bun.file(stampPath).text()));
  } catch {
    // A missing or malformed stamp only means the next launch makes a request.
    return undefined;
  }
}

async function writeSyncStamp(stampPath: string, stamp: PiMcpSyncStamp): Promise<void> {
  try {
    await mkdir(dirname(stampPath), { recursive: true, mode: 0o700 });
    const temporaryPath = `${stampPath}.${process.pid}.${crypto.randomUUID()}.tmp`;
    try {
      await Bun.write(temporaryPath, `${JSON.stringify(stamp, null, 2)}\n`);
      await chmod(temporaryPath, DEFAULT_FILE_MODE);
      await rename(temporaryPath, stampPath);
    } finally {
      await rm(temporaryPath, { force: true });
    }
  } catch {
    // Best effort: a stamp that cannot be written costs one extra request later.
  }
}

export function isMcpSyncThrottled(
  stamp: PiMcpSyncStamp | undefined,
  sourceUrl: string,
  now: Date,
): boolean {
  if (stamp === undefined || stamp.sourceUrl !== sourceUrl) {
    return false;
  }
  const attemptedAt = Date.parse(stamp.lastAttemptAt);
  if (!Number.isFinite(attemptedAt)) {
    return false;
  }
  const age = now.getTime() - attemptedAt;
  const interval =
    stamp.lastStatus === "ok" ? PI_MCP_SYNC_SUCCESS_INTERVAL_MS : PI_MCP_SYNC_FAILURE_INTERVAL_MS;
