...
export async function launchRegisteredHarness(
  ...
  const resolvedExecutable = await resolveExecutablePath(registration, inheritedEnvironment, cwd);
  const adapter = await loadHarnessAdapter(registration.id);

  const context: HarnessLaunchContext = Object.freeze({
    cwd,
    environment: inheritedEnvironment,
    paths: resolveDeputyDevPaths(inheritedEnvironment),
    registration,
  ...
  if (specification.executable !== resolvedExecutable.path) {
    throw new LauncherError(
