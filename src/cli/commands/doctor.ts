...
async function inspectHarness(
  ...
  try {
    const executable = await resolveExecutablePath(registration, context.environment, context.cwd);
    const version = await probeVersion(executable.path, context.environment);
    ...
    return {
      findings: [
        {
          level: "info",
          message: `${registration.id}: executable ${executable.path} (${executable.source})`,
        },
        version === undefined
          ? {
              level: "warning",
