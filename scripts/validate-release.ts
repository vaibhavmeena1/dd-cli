...
export async function validateRelease(): Promise<void> {
  ...
  if (details.tag !== expectedVersion) {
    throw new Error(
      `release tag ${details.tag} does not equal requested version ${expectedVersion}`,
    );
  }
  if (details.name !== details.tag) {
    throw new Error(`release name ${details.name} must equal release tag ${details.tag}`);
  }
  if (packageVersion !== details.tag) {
    throw new Error(`package.json version ${packageVersion} must equal release tag ${details.tag}`);
  }
